...
$.fn.treeSelection = function (options) {
    ...
    let data_grid = $('<div>')
        ...
        .css({
            ...
            'top': (this.position().top + parseInt(String(parseInt(this.css('height')) - 40))),
    ...
    let getCode = function (data) {
        ...
        for (let i = 0, m = data.length; i < m; i++) {
            if (typeof data[i] === 'object' && typeof data[i].items != 'undefined') {
                result += '<li>';
                result += '<span class="preIcon arrowDown arrowRotate"></span>';
    ...
    $('.preIcon.arrowDown').click(function () {
        list_sel($(this));
    });

    function list_sel($toggle) {
        let $ul = $toggle.siblings('ul');
        if ($ul.is(':visible')) {
            $toggle.addClass('arrowRotate');
            $ul.slideUp();
        } else {
            $toggle.removeClass('arrowRotate');
            $ul.slideDown();
        }
